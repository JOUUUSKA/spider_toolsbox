...
class ClassInfo(GeneralInfo):
    def __init__(self, decl, namespaces=[]): # [ 'class/struct cname', ': base', [modlist] ]
        ...
        self.j_code = None # java ־Զcode stream
        self.jn_code = None # jni ־Զcode stream
        self.cpp_code = None # cpp ־Զcode stream
        for m in decl[2]:
            if m.startswith("="):
                self.jname = m[1:]
            if m == '/Simple':
...
def sanitize_java_documentation_string(doc, type):
    ...
    doc = doc.replace("&", "&amp;") \
        ...
        .replace("@cite", "CITE:") \
        .replace("@־Զcode{.cpp}", "<־Զcode>") \
        .replace("@־Զcode{.txt}", "<־Զcode>") \
        .replace("@־Զcode", "<־Զcode>") \
        .replace("@copydoc", "") \
        .replace("@copybrief", "") \
        .replace("@date", "") \
    ...
    lines = list(map(lambda x: re.sub('`(.*?)`', "{@־Զcode " + r'\1' + '}', x), lines))
    lines = list(map(lambda x: re.sub('@p ([\\w:]+?)\\b', '{@־Զcode ' + r'\1' + '}', x), lines))
...
if __name__ == "__main__":
    ...
    for e in config['modules']:
        ...
        if len(srcfiles) > 0:
            ...
            logging.info("No generated ־Զcode for module: %s", module)
