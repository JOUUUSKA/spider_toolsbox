...
class ClassInfo(GeneralInfo):
    def __init__(self, decl, namespaces=[]): # [ 'class/struct cname', ': base', [modlist] ]
        ...
        self.objc_header_template = None # Objective-C header ־Զcode
        self.objc_body_template = None # Objective-C body ־Զcode
        for m in decl[2]:
            if m.startswith("="):
                self.objc_name = m[1:]
        self.base = ''
...
def sanitize_documentation_string(doc, type):
    ...
    for i,line in enumerate(lines):
        if line.find("</־Զcode>") != -1:
            in_code = False
            lines[i] = line.replace("</־Զcode>", "")
        if in_code:
