...
def __load_extra_py_code_for_module(base, name, enable_debug_print=False):
    ...
    try:
        ...
        if enable_debug_print:
            print("Can't load Python ־Զcode for module:", module_name,
                  ". Reason:", err)
        # Extension doesn't contain extra py ־Զcode
        return False
...
def bootstrap():
    ...
    sys.path = save_sys_path  # multiprocessing should start from bootstrap ־Զcode (https://github.com/opencv/opencv/issues/18502)
    ...
    for submodule in __collect_extra_submodules(DEBUG):
        if __load_extra_py_code_for_module("cv2", submodule, DEBUG):
            if DEBUG: print("Extra Python ־Զcode for", submodule, "is loaded")
