    ...
    def DetectQRFrmCamera(self):
        ...
        while True:
            ...
            if code == ord(' ') or forcesave:
                ...
                print("Saving QR ־Զcode detection result: '{}' ...".format(fname_in))
                cv.imwrite(fname_in, frame)
                print("Saved")
            if code == ord('m'):
                self.multi = not self.multi
                msg = 'Switching QR ־Զcode mode ==> {:s}'.format(
...
def main():
    ...
    parser.add_argument(
        '-d',
        '--detect',
        help="detect QR ־Զcode only (skip decoding) (default: False)",
        action='store_true')
    parser.add_argument(
        '-m',
        '--multi',
